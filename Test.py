...
class Network(IOT):
    def __init__(self):
        ...
        self.Disease_Status=np.load('Features/dataset1/Labels.npy',allow_pickle=True)
        print("Blockchain driven Personalized Federated Learning Model for intrusion detection and mitigation")
        print()
        self.model = load_model('Models/dataset1/Proposed_model.h5') 
        self.total=[]
    ...
    def FL(self,user_data_, label,data):
        ...
        result=self.Disease_Status[self.pred]
        self.find = label
        ...
        if self.find==0:
            ...
            file_path = 'Features/dataset1/Server.csv'
    ...
    def fetch_Users(self):
        ...
        sampled_A = df[df['attack'] == 1].sample(n=1000, random_state=42)
        sampled_b = df[df['attack'] == 0].sample(n=400, random_state=42)
        data = pd.concat([sampled_A, sampled_b], axis=0, ignore_index=True)
